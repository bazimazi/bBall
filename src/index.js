...
function update(dt) {
  ...
  ball.vx = dirX * currentBallSpeed;             // scale X by difficulty
  ball.vy = dirY * Math.min(Math.abs(ball.vy), currentBallSpeed); // keep Y sane
  ...
  if (ball.x + ball.radius < 0) {
    scoreBot++;
    resetBall(-1);           // bumps baseDifficulty and resets difficultyMultiplier to it
...
// Reset the ball AND reset difficulty to a bumped base each score.
// bumpBase = true for score resets, false for full game reset.
function resetBall(direction = 1, bumpBase = true) {
  if (bumpBase) {
    baseDifficulty += BASE_DIFF_INC_PER_RESET;
  }
  difficultyMultiplier = baseDifficulty; // <- set current difficulty to the (possibly increased) base
...
function resetGame() {
  ...
  resetBall(Math.random() > 0.5 ? 1 : -1, /* bumpBase */ false);
  gameState = 'playing';
}
